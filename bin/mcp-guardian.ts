...
<body>
  <div class="container">
    ...
    <div class="footer">
      Generated by <a href="https://github.com/alexandriashai/mcp-guardian">mcp-guardian</a> v${VERSION}
    </div>
  </div>
</body>
</html>`;
    ...
    for (const server of filteredResult.servers) {
      ...
      if (server.issues.length > 0) {
        for (const tool of server.issues) {
          ...
          for (const issue of tool.issues) {
            const severity = issue.severity === "critical" ? "🚨 CRITICAL" : "⚠️  WARNING";
